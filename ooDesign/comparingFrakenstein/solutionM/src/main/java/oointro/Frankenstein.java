...
public class Frankenstein implements Comparable<Frankenstein> {
    ...
    // Method required to use comparable interface
    @Override
    public int compareTo(Frankenstein f) {
        double selfAvg = getAlphaAvg();
        double compAvg = f.getAlphaAvg();

        if (selfAvg > compAvg) {
            return 1;
        } else if (selfAvg < compAvg) {
            return -1;
        } else {
            return 0;
        }
